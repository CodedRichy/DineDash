                            ...
                            {menu.map(item => (
                                <div key={item.id} className={`bg-white rounded-3xl p-6 border shadow-sm flex flex-col hover:shadow-md transition-all ${!item.is_available ? 'opacity-50 grayscale' : 'border-gray-100'}`}>
                                    <div className="flex justify-between items-start mb-4">
                                        ...
                                        <span className="font-black text-xl text-green-600 bg-green-50 px-3 py-1 rounded-xl">${item.price}</span>
                                    </div>
                                    <p className="text-gray-500 font-medium text-sm mb-6 flex-grow">{item.description}</p>
